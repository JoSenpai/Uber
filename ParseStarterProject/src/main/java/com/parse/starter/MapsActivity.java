...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void checkForUpdates()
    {
        ...
        //if the request belongs to you, the rider
        query.whereEqualTo("username", ParseUser.getCurrentUser().getUsername());

        //if the request has an existing driver
                                ...
                                if(Build.VERSION.SDK_INT < 23 || ContextCompat.checkSelfPermission(MapsActivity.this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                                    ...
                                    if(lastKnownLocation != null)
                                    {
                                        ...
                                        if(distanceKM < 0.02)
                                        {
                                            ...
                                            //recalls this function every 2 second, checking for updates
                                        ...
                                        }else
                                        {
                                            ...
                                            //MAP HAS TO BE READY BEFORE ADDING THE STUPID MARKER OR WILL CRASH
                                            markers.add(mMap.addMarker(new MarkerOptions().position(driverLocationLatLng).title("Driver Location").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE))));
                                            ...
                                            /// cant cancel the ride when driver is on the way!ow t
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //checks if there is any current request in Parse server
        ParseQuery<ParseObject> query = new ParseQuery<ParseObject>("Request");
        //if there is a request for the current user logged in
        query.whereEqualTo("username",ParseUser.getCurrentUser().getUsername());
        query.findInBackground(new FindCallback<ParseObject>() {
            @Override
            public void done(List<ParseObject> objects, ParseException e) {
                if(e == null)
                {
                    ...
                    if(objects.size() > 0)
                    {
                        //it is still requesting for an uber
                        requestActive = true;
    ...
    public void callUber(View view)
    {
        ...
        //if currently requesting an uber, allow to cancel the uber
        if(requestActive)
        {
            ParseQuery<ParseObject> query = new ParseQuery<ParseObject>("Request");
            query.whereEqualTo("username",ParseUser.getCurrentUser().getUsername());
                ...
                public void done(List<ParseObject> objects, ParseException e) {
                    if(e == null)
                    {
                        if(objects.size() > 0)
                        {
                            for(ParseObject object : objects)
                            {
                                //removes the request in the parse server
                                object.deleteInBackground();
                            }
                            requestActive = false;
