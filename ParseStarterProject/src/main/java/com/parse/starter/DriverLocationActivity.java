...
public class DriverLocationActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    {
        Log.i("Accept Request", "called");
        ParseQuery<ParseObject> query = ParseQuery.getQuery("Request");

        //matches the request with the username
            ...
            public void done(List<ParseObject> objects, ParseException e) {
                if(e == null)
                {
                    if(objects.size() > 0)
                    {
                        ...
                        {
                            //tells parse there is a driver who accepted the request
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mapLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                markers.add(mMap.addMarker(new MarkerOptions().position(driverLocation).title("Your Location").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE))));
